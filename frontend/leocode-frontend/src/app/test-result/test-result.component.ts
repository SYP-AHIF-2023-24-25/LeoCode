...
export class TestResultComponent  implements OnInit{
 ...
 resultHistory:{message: string,timestamp:Date, passed:number, notPassed:number, total:number, timer:string}[] = [];
  ...
  calculateEditorHeight(code: string): string {
    const lineHeight = 19; // Standardhöhe einer Zeile im Monaco Editor
    const minLines = 3;    // Mindestanzahl Zeilen
    const maxLines = 30;   // Maximal erlaubte Zeilenanzahl
  
    const lines = code ? code.split('\n').length : minLines;
    const height = Math.min(Math.max(lines, minLines), maxLines) * lineHeight;
